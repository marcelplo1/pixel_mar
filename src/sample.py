...
from src.utils import sample_order, unpatchify, save_img_as_fig
...
@torch.no_grad()
def sample(denoising_mlp, mae, bsz, seq_len, embed_dim, device, pred_type = "x", num_iter=64, num_timesteps=32, debug=False):
    xt = torch.randn((bsz, seq_len, embed_dim), device=device)
    tokens = torch.zeros_like(xt)
    mask = torch.ones(bsz, seq_len, device=device)
    orders = sample_order(bsz, seq_len)
    ...
    for t_idx in range(num_timesteps):
        ...
        x_i = x_i + (t_next - t) * v_pred

    xt[mask_to_pred.nonzero(as_tuple=True)] = x_i
    tokens[mask_to_pred.nonzero(as_tuple=True)] = x_i

    if debug:
        save_img_as_fig(tokens, filename="sample.png", path="./output")
    
    img = unpatchify(tokens, int(math.sqrt(embed_dim)), seq_len)
