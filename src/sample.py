...
def sample(args, mae, denoiser, labels, device, model_params, sampler_params):
    ...
    tokens = torch.zeros(bsz, channels, img_size, img_size, device=device)
    ...
    for i in range(num_ar_steps):
        ...
        z, mask, _ = mae(tokens, orders, labels, num_visible)
        ...
        if args.use_logging and local_rank == 0:
            ...
            save_img_as_fig(tokens, file_path=file_path, size=img_size)
