...
@torch.no_grad()
def sample(args, ar_model, denoiser, labels, device, model_params, sampler_params, tokenizer=None):
    ...
    use_latent = tokenizer is not None

    seq_len = (img_size// patch_size) ** 2

    if use_latent:
        embed_dim = tokenizer.latent_dim
    else:
        embed_dim = (patch_size ** 2) * channels

    cur_tokens = torch.zeros(bsz, seq_len, embed_dim, device=device)
    orders = sample_order(bsz, seq_len, device, strategy=sampler_params.get('order_strategy', 'random'))
    ...
    if args.use_logging and local_rank == 0:
        ...
        existing = [d for d in os.listdir(folder_steps) if d.startswith("run_")]
        run_idx = len(existing) % 5 #Log 5 different generation processes
        run_folder_steps = os.path.join(folder_steps, f"run_{run_idx}")
        run_folder_z_pca = os.path.join(folder_z_pca, f"run_{run_idx}")
        os.makedirs(run_folder_steps, exist_ok=True)
        os.makedirs(run_folder_z_pca, exist_ok=True)
    else:
        run_folder_steps = run_folder_z_pca = None
    ...
    for i, next_num_visible in enumerate(schedule):
        ...
        # CFG schedule:
        cfg_schedule = sampler_params.get('cfg_schedule', 'progressive')
        if cfg_schedule == 'linear':
            cfg_scale_iter = cfg_scale
        elif cfg_schedule == 'progressive':
        ...
        # Log intermediate steps during autoregressive generation
        if args.use_logging and local_rank == 0:
            save_pca_viz(z_cond, os.path.join(run_folder_z_pca, "step_{}.png".format(i)), img_size, idx=0)
            if use_latent:
                save_img_as_fig(tokenizer.decode(cur_tokens),
                                file_path=os.path.join(run_folder_steps, "step_{}.png".format(i)), size=img_size)
            else:
                save_img_as_fig(unpatchify(cur_tokens, patch_size, channels=channels),
                                file_path=os.path.join(run_folder_steps, "step_{}.png".format(i)), size=img_size)

    if use_latent:
        img = tokenizer.decode(cur_tokens)
    else:
